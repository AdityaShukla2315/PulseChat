...
export const useChatStore = create((set, get) => ({
    ...
    const handleNewMessage = (newMessage) => {
      ...
      set((state) => {
        try {
          ...
          // If a temp message exists for this sender/createdAt, replace it
          const tempIndex = state.messages.findIndex(msg =>
            msg._id && typeof msg._id === 'string' && msg._id.startsWith('temp_') &&
            msg.senderId === newMessage.senderId &&
            Math.abs(new Date(msg.createdAt).getTime() - new Date(newMessage.createdAt).getTime()) < 5000 // 5s window
          );
          let updatedMessages = [...state.messages];
          if (tempIndex !== -1) {
            updatedMessages[tempIndex] = { ...newMessage, isSending: false, error: false };
            return { messages: updatedMessages };
          }
          ...
          console.error('Message that caused error:', newMessage);
          return {}; // Return empty update to prevent crash
        }
      });
    };
  ...
  setSelectedUser: (selectedUser) => {
    // Remove preview and unread for this user
    set((state) => ({
      selectedUser,
      users: state.users.map(u =>
        u._id === selectedUser._id
          ? { ...u, lastMessage: '', lastMessageTime: null, unread: false }
          : u
      )
    }));
  },
