...
export const sendMessage = async (req, res) => {
  ...
  try {
    ...
    if (text && text.trim()) {
      try {
        console.log(`[${requestId}] Moderating message content...`);
        moderationResult = await aiService.moderateContent(text);
        
        if (moderationResult.isToxic && moderationResult.confidence > 0.7) {
          console.log(`[${requestId}] Message flagged as toxic:`, moderationResult);
        ...
        console.warn(`[${requestId}] Content moderation failed:`, moderationError.message);
    ...
    // Emit to receiver if online (but not to sender to avoid duplicate)
    if (receiverSocketId) {
      console.log(`[${requestId}] Emitting newMessage to receiver:`, receiverSocketId);
      io.to(receiverSocketId).emit("newMessage", messageData);
      console.log(`[${requestId}] Message emitted to receiver successfully`);
    } else {
      console.log(`[${requestId}] Receiver ${receiverId} is offline, message will be delivered on next connection`);
    }
